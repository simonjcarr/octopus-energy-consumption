...
export default async function handler(
  ...
  let weekOffset: any = req.query.weekOffset
  
  const period_from = DateTime.now().setZone('Europe/London').minus({ days: weekOffset * 7 }).endOf('day').startOf('second').toISO().replace(/\.000\+00:00/, "Z")
  console.log(period_from)
  
  const period_to = DateTime.now().setZone('Europe/London').minus({ days: (weekOffset * 7) - 7 }).endOf('day').startOf('second').toISO().replace(/\.000\+00:00/, "Z")
  console.log(period_to)
  const result = await fetch(`https://api.octopus.energy/v1/electricity-meter-points/${MPAN}/meters/${SERIAL}/consumption/?period_from=${period_from}&period_to=${period_to}&page_size=${48*7}`,
    {
      headers: new Headers({
